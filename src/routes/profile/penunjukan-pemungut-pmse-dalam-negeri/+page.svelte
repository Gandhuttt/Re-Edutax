<script lang="ts">
	...
	const businessModels = ['Lokapasar (marketplace)', 'Ritel daring', 'Iklan baris daring', 'Platform berbagi', 'Lainnya'];
...
<svelte:head><title>Penunjukan Pemungut PMSE Dalam Negeri</title></svelte:head>

<div class="page-shell">
	<section class="card">
		<header class="card-header"><h1>Penunjukan Pemungut PMSE Dalam Negeri</h1></header>
		<form class="card-body" onsubmit={submit} oninput={() => { saved = false; error = ''; }} novalidate>
			...
			<fieldset>
				<legend>Data Perdagangan Melalui Sistem Elektronik</legend>
				<div class="form-grid">
					<label>Nama Sistem Elektronik *<input required /></label>
					<label>Model Bisnis *<select required><option value="">Silakan Pilih</option>{#each businessModels as model}<option>{model}</option>{/each}</select></label>
					<label class="full">Alamat Situs Web atau Aplikasi *<input type="url" required placeholder="https://" /></label>
					<label>Tanggal Mulai Kegiatan PMSE *<input type="date" required /></label>
					<label>Jumlah Pengakses di Indonesia per Tahun *<input type="number" min="0" step="1" required /></label>
					<label>Nilai Transaksi dengan Pembeli Indonesia per Tahun (Rp) *<input type="number" min="0" step="1" required /></label>
					<label>Jumlah Transaksi dengan Pembeli Indonesia per Tahun *<input type="number" min="0" step="1" required /></label>
			...
			<fieldset>
				...
				<div class="upload-grid">
					<label>Surat Permohonan *<input type="file" accept=".pdf" required /></label>
					<label>Dokumen Izin atau Profil Usaha *<input type="file" accept=".pdf,.jpg,.jpeg,.png" required /></label>
				</div>
			...
			{#if saved}<p class="message success" role="status">Permohonan penunjukan pemungut PMSE dalam negeri telah disimpan.</p>{/if}
