<script lang="ts">
	...
	function filterRows(
		rows: Awaited<ReturnType<typeof listBp26>>,
...
<div class="tw:w-full tw:p-25">
	<div class="tw:text-2xl tw:h-10 tw:flex tw:items-center tw:mb-3">eBupot BP26</div>
	...
	<div class="tw:flex tw:flex-row tw:gap-5">
		<div class="tw:w-[12rem] tw:shrink-0 tw:rounded-sm tw:bg-gray-100 tw:border tw:border-[#a9a9a9]">
			<div class="tw:px-3 tw:py-2 tw:font-semibold tw:border-b tw:border-b-[#a9a9a9]">BP26</div>
		...
		<div class="tw:grow tw:min-h-100 tw:overflow-scroll tw:rounded-sm tw:bg-gray-100 tw:border tw:border-[#a9a9a9]">
			...
			>
				<span class="tw:uppercase">EBUPOT BP26 {activeTab}</span>
				{#if activeTab === 'Belum Terbit'}
					<form {...newEmpty}><Button type="submit">+ Create eBupot BP26</Button></form>
				{/if}
			</div>
			<Table class="tw:w-full">
				...
				{/snippet}
				{#snippet body()}
					{#each filterRows(await listBp26(), activeTab) as row}
						{@const hapusBp26 = deleteBp26.for(row.id)}
						{@const terbitkanRow = terbitkanBp26.for(row.id)}
						<tr>
							<td>
								<div class="tw:flex tw:flex-row tw:gap-1">
									<a href="/ebupot/bp26/{row.id}" class="tw:text-black!">
										<Button>Buka</Button>
									</a>
									{#if !row.diterbitkan}
